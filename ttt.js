    ...
    if (tile === EMPTY_TILE_SYMBOL) {
        ...
        if (gameOver()) {
            ...
            if (isPlayerTurn) {
                ...
                gameResult.innerHTML = `<span style='color: ${SECOND_PLAYER_SYMBOL}> Enemy </span> has won!`
            }
        ...
        if (!isPlayerTurn && is isThereAnEmptyCell()) {
            randomComputerMove()
        }
...
function isVerticalWin() {
    ...
    for (let col = 0; col < CELLS_VERTICALLY; col++) {
        ...
        for (let row = 0; row < gameBoard.length; row++) {
            let tile = gameBoard[row][col]
            if (tile === FIRST_PLAYER_SYMBOL) {colSum += playerVaue}
            else if (tile === SECOND_PLAYER_SYMBOL) {colSum += enemyValue}
            
        }
        if (colSum === playerVaue * CELLS_HORIZONTALLY || colSum === enemyValue * CELLS_VERTICALLY ) {
            return true
        }
...
function isDiagonalWin() {
    ...
    let firstDiagSum = 0
    let secondDiagSum = 0
    for (let row = 0, col = 0, rowEnd = CELLS_HORIZONTALLY - 1; row < CELLS_VERTICALLY && col < CELLS_HORIZONTALLY; row++, col++, rowEnd--) {
