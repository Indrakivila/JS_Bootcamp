...
function isTeen(first, second, third) {
    ...
    return (first >= 13 && first <= 19) || (second >= 13 && second <= 19 ) || (third >= 13 && sthird <= 19 ) 
}
...
function mixStart(s) {
    // let firstTwoLetters = s[1] + s[2]
    // if (firstTwoLetters === 'ix') {
    //     return true
    // } else {
    //     return false

    // }
    let firstTwoLetters = s.slice(1, 3)
    return firstTwoLetters === 'ix'
}
...
function makeOutWord(pattern, text) {
    let patternSlice = pattern[0] + pattern[1] 
    let patternSliceTwo = pattern[2] + pattern[3]
        return patternSlice + text + patternSliceTwo
...
function reverseArray(arr = [1,2,3]) {
    let reversedArray = [] 
...
function lucky1(arr) {
    for (let i = 0; i < arr.length; i++) {
        ...
        let isInRange =  i === 0 || i === 1 || i === arr.length - 1 || i === arr.length - 2
...
function front11(arrOne, arrTwo) {
    let newArray = []
    if (arrOne.length != 0) {
        newArray.push(arrOne[0])
    }
    if (arrTwo.length != 0) {
        newArray.push(arrTwo[0])
    } 
    return newArray
}
...
// console.log(getFullName("Indra", "Kivila"))
