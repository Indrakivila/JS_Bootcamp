...
const OPERATION_SUTRACION = 'subtraction'
...
const OPERATION_MODOLO = ' modulo'

const MAX_NUMBERS_AFTER_DECIMAL_POINT = 5
const MAX_INPUT_LENGTH = 20
const ERROR_MSG_INPUY_TOO_LONG = 'Too long!'
...
function setInputNumber(button) {
    ...
    if (result.innerText === ERROR_MSG_INPUY_TOO_LONG) { return }
    if (tooLong(firstNumber) || tooLong(secondNumber)) {
        result.innerText = ERROR_MSG_INPUY_TOO_LONG
        return
    }
    ...
    if (firstNumber.length > 0 && secondNumber.length >0 && operation) {
        switch (operation) {
            ...
            case OPERATION_MODULO:
                ...
                break;
            default:
                alert('Unrecognational operation: ' + operation)
        }
        ...
        // if (split.length === 2) {
        //     if (split[1].length > MAX_NUMBERS_AFTER_DECIMAL_POINT) {
        //         split[1] = parseFloat(split[1]).toFixed().toString() 
        //     }
        // }
        result.innerText = firstNumber
